...
internal class OptimizedKvsExtended(
    ...
    override suspend fun getString(key: String, defValue: String): String =
        withContext(dispatcher) { getOrDefault(key, defValue, String::toString) }

    override suspend fun getInt(key: String, defValue: Int): Int =
        withContext(dispatcher) { getOrDefault(key, defValue, String::toIntOrNull) }

    override suspend fun getLong(key: String, defValue: Long): Long =
        withContext(dispatcher) { getOrDefault(key, defValue, String::toLongOrNull) }

    override suspend fun getFloat(key: String, defValue: Float): Float =
        withContext(dispatcher) { getOrDefault(key, defValue, String::toFloatOrNull) }

    override suspend fun getBoolean(key: String, defValue: Boolean): Boolean =
        withContext(dispatcher) { getOrDefault(key, defValue, String::toBoolean) }
    ...
    override fun getStringAsStream(key: String, defValue: String): Flow<String> =
        queries.selectByKey(key).asFlow().mapToOneOrNull(dispatcher)
    ...
    override fun getIntAsStream(key: String, defValue: Int): Flow<Int> =
        queries.selectByKey(key).asFlow().mapToOneOrNull(dispatcher)
            .map { row ->
                if (row == null || row.isExpiredNow()) defValue else row.value_.toIntOrNull()
                    ?: defValue
            }
    ...
    override fun getLongAsStream(key: String, defValue: Long): Flow<Long> =
        queries.selectByKey(key).asFlow().mapToOneOrNull(dispatcher)
            .map { row ->
                if (row == null || row.isExpiredNow()) defValue else row.value_.toLongOrNull()
                    ?: defValue
            }
    ...
    override fun getFloatAsStream(key: String, defValue: Float): Flow<Float> =
        queries.selectByKey(key).asFlow().mapToOneOrNull(dispatcher)
            .map { row ->
                if (row == null || row.isExpiredNow()) defValue else row.value_.toFloatOrNull()
                    ?: defValue
            }
    ...
    override fun edit(): KvsExtended.KvsExtendedEditor =
        OptimizedKvsExtendedEditor(queries, ttlManager)
...
private class OptimizedKvsExtendedEditor(
    ...
    override fun putString(key: String, value: String, duration: Duration) =
        put(key, value, duration)

    override fun putInt(key: String, value: Int) = put(key, value.toString())
    override fun putInt(key: String, value: Int, duration: Duration) =
        put(key, value.toString(), duration)

    override fun putLong(key: String, value: Long) = put(key, value.toString())
    override fun putLong(key: String, value: Long, duration: Duration) =
        put(key, value.toString(), duration)

    override fun putFloat(key: String, value: Float) = put(key, value.toString())
    override fun putFloat(key: String, value: Float, duration: Duration) =
        put(key, value.toString(), duration)

    override fun putBoolean(key: String, value: Boolean) = put(key, value.toString())
    override fun putBoolean(key: String, value: Boolean, duration: Duration) =
        put(key, value.toString(), duration)
